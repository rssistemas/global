...
/*      $.post('/pdval/archivo/proveedor/buscarProveedor/','valor=' + valor,function(datos){
        ...
        $.post('/globalAdm/compra/proveedor/comprobarUso/', 'valor=' + valor, function (resultado) {
...
/*******PARA ELIMINAR LOS REGISTROS DEL LISTADO DE LA VISTA PRINCIPAL**********/
    var eliminar = function(ref){
        $.post('/globalAdm/compra/proveedor/comprobarUso/', 'valor=' + ref, function (resultado) {
            if (resultado.total > 0)
            {
            ...
            {
                $.post('/globalAdm/compra/proveedor/estatusProveedor/','valor='+ref +'&estatus='+'9', function (filas){
    ...
    var restaurar = function(ref){
        ...
        {
            $.post('/global/compra/proveedor/estatusProveedor/','valor='+ref +'&estatus='+'1', function (filas){
                document.location.reload();
            }, 'json');
        }
    };
...
//---------------------------------------------------------------------------------------
    $('#nacionalidad').change(function(){
        ...
        if($('#nacionalidad').val()!='0' && $('#cedula').val()!='' )
            if($('#cedula').val()>1000000)
            {
                if($('#nacionalidad').val()!='0' && $('#cedula').val()!='' )
                {
                    $.post('/global/compra/proveedor/comprobarProveedor/','tipo=' + $("#nacionalidad").val()+'&rif='+ $("#cedula").val(),function(datos){
    ...
    $('#cedula').change(function(){
        ...
        if($('#cedula').val()>1000000)
        {
            if( $('#nacionalidad').val()!='0' && $('#cedula').val()!='' )
            {
                $.post('/globalAdm/compra/proveedor/comprobarProveedor/','tipo=' + $("#nacionalidad").val()+'&rif='+ $("#cedula").val(),function(datos){
                    if(!datos.total==0) //si ya existe un deposito con el mismo nombre y tipo
                    {
                        alert("El número de rif que intenta registrar ya existe, no puede registrado nuevamente.");
                        document.getElementById('cedula').focus();
    ...
    var getMunicipio = function(){
        $.post('/globalAdm/configuracion/municipio/buscarMunicipios/','valor='+$("#estado").val(),function(datos){
            $('#municipio').html('');
            $('#municipio').append('<option value="0" >-Seleccione-</option>');
            $('#parroquia').html('');
            $('#parroquia').append('<option value="0" >-Seleccione-</option>');
    ...
    var getParroquia = function(){
        $.post('/globalAdm/configuracion/parroquia/buscarParroquias/','valor='+$("#municipio").val(),function(datos){
                $('#parroquia').html('');
                $('#parroquia').append('<option value="0" >-Seleccione-</option>');
                $('#sector').html('');
                $('#sector').append('<option value="0" >-Seleccione-</option>');
    ...
    var getSector = function(){
        $.post('/globalAdm/configuracion/sector/buscarSectores/','valor='+$("#parroquia").val(),function(datos){
            $('#sector').html('');
                $('#sector').append('<option value="0" >-Seleccione-</option>');
            if(datos.length > 0)
            {
