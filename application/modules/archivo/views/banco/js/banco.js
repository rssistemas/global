...
/*** METODOS PARA INTERACTUAR CON EL CONTROLADOR DE LA VISTA ***/
    ...
    var setDatos = function(){
        ...
        else
        {
            ...
            {
                $.post('/globalAdm/archivo/banco/comprobarBanco/','valor=' + $("#nombre").val(),function(cantidad){
                    if( cantidad.total==0 )
                    {
                        if(confirm("¿Realmente desea guardar el Banco ?"))
                        {
                            $("#banco").submit();
                        }
                    }
                    if( cantidad.total>=1 )
                    {
                        alert("El Banco que intenta registrar ya existe, no puede registrado nuevamente.");
                        document.getElementById('nombre').focus();
                    }
                },'json');
            }
            if($('#guardar').val()==2) // para guardar la edicion del registro
            {
                ...
                {
                    if(confirm("¿Realmente desea editar el Banco ?"))
                    {
                        $("#banco").submit();
                    }
                }
    ...
    var getDatos = function(valor){
        $.post('/condominio/archivo/tipoDocumento/buscarTipoDocumento/','valor=' + valor,function(datos){
            if(datos)
            {
                limpiar_formulario();
				...
				$('$comentario').val(datos.comentario_tipo_documento);
        ...
        $.post('/pdval/configuracion/tipoDocumento/comprobarUso/', 'valor=' + valor, function (resultado) {
