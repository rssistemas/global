...
/*** METODOS PARA INTERACTUAR CON EL CONTROLADOR DE LA VISTA ***/
    ...
    var setDatos = function(){
        ...
        else
        {
            ...
            {
                $.post('/pdval/configuracion/tipoDocumento/comprobarTipoDocumento/','valor=' + $("#nombre").val(),function(cantidad){
                    if( cantidad.total==0 )
                    {
    ...
    var getDatos = function(valor){
        $.post('/pdval/configuracion/tipoDocumento/buscarTipoDocumento/','valor=' + valor,function(datos){
            if(datos)
            {
                limpiar_formulario();
...
/*    var eliminar = function(ref){
    ...
    var restaurar = function(ref){
        ...
        {
                $.post('/pdval/configuracion/tipoDocumento/estatusTipoDocumento/','valor='+ ref+'&estatus='+'1',function(filas){
                document.location.reload();
            }, 'json');
        }
    };
    ...
    var eliminar = function(ref){
        $.post('/pdval/configuracion/tipoDocumento/comprobarUso/', 'valor=' + ref, function (resultado) {
            if (resultado.total > 0)
            {
                alert("El registro ya se encuentra en uso, no puede ser eliminado.");
            }
