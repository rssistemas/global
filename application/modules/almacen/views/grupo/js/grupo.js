...
/*** Llamado al botones tipo clase desde el nombre de la clase (.nombre_clase) ***/
    ...
    $(".nuevo").click(function(e){
    	if(confirm("Desea crear nuevo grupo")){
...
/*** METODOS PARA INTERACTUAR CON EL CONTROLADOR DE LA VISTA ***/
    ...
    var setDatos = function () {
        ...
        else
        {
            ...
            {
                $.post('/globalAdm/almacen/grupo/comprobarGrupo/', 'valor=' + $("#descripcion").val(), function (cantidad) {
                    if (cantidad.total == 0)
                    {
                        if (confirm("¿Realmente desea guardar el nuevo grupo para producto?"))
                        {
                            $("#rubro").submit();
                        }
                    }
    ...
    var getDatos = function (valor) {
        $.post('/globalAdm/almacen/grupo/buscarGrupo/', 'valor=' + valor, function (datos) {
            if(datos)
            {
                limpiar_formulario();
    ...
    var eliminar = function (ref) {        
        $.post('/globalAdm/almacen/grupo/comprobarGrupo', 'valor=' + ref, function (resultado) {
            ...
            {
                $.post('/globalAdm/almacen/grupo/estatusRubro/','valor='+ref +'&estatus='+'9', function (filas){
    ...
    var restaurar = function(ref){
        ...
        {
            $.post('/globalAdm/almacen/grupo/estatusRubro/','valor='+ref +'&estatus='+'1', function (filas){
                document.location.reload();
            }, 'json');
        }
    };
