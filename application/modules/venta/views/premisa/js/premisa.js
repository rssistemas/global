...
/*** Llamado al botones tipo clase desde el nombre de la clase (.nombre_clase) ***/
    ...
    var setDatos = function () {
        ...
        else
        {
            ...
            {                
				if (confirm("¿Realmente desea guardar la nueva premisa ?"))
				{
					$("#form_premisa").submit();
				}      
            }
    ...
    var getDatos = function (valor) {
		$.ajax( 
			{  
				url: '/globalAdm/venta/premisa/buscarPremisa/',
				type: 'POST',
				dataType : 'json',
				async: false,
				data: 'valor='+valor,
					  ...
					  if(datos)
						{
							...
							$('#id').val(datos.id_premisa);
							$('#nombre').val(datos.nombre_premisa);
							$('#descripcion').val(datos.descripcion_premisa);
							$('#porcentaje').val(datos.porcentaje_premisa);
							$('#valor').val(datos.comparador_premisa);
							//$('#comentario').val(datos.comentario_premisa);
							$('#condicion').val(datos.tipo_premisa+' '+datos.operador_premisa+' '+datos.comparador_premisa);
							$("#operador option[value="+ datos.operador_premisa +"]").attr("selected",true);
							$("#accion option[value="+ datos.accion_premisa +"]").attr("selected",true);
